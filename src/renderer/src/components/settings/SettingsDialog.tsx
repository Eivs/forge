  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[850px] max-h-[75vh] overflow-hidden p-0">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col h-[calc(75vh-60px)]">
          <TabsList className="px-4 mb-1">
            <TabsTrigger value="general" className="text-xs py-1">{t.settings.general}</TabsTrigger>
            <TabsTrigger value="providers" className="text-xs py-1">{t.settings.providers}</TabsTrigger>
            <TabsTrigger value="mcp" className="text-xs py-1">MCP</TabsTrigger>
          </TabsList>
